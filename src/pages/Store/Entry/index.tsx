...
const Products = () => {
    ...
    const handelnameChange     = (event: React.ChangeEvent<HTMLInputElement> ) => {setSelectedName(event.target.value);};

    const getProducts = async() => {
        let query= "";
        if (selectedLocation){ query += `&location_id=${selectedLocation}`; }
        if (selectedcategory){ query += `&category_id=${selectedcategory}` }
        if (selectedName.trim()){ query += `&name=${selectedName.trim()}` }
        if (!query){toast.error("😒 Please select atleast one filter "); return false}
        query.substring(1);
        const response = await getFromServer(`/store/products?${query}`);
    ...
    <>
        <div className="w-full p-4 bg-white shadow-default dark:border-strokedark dark:bg-boxdark rounded-lg flex flex-col md:flex-row items-center gap-4">
            ...
            <input type="search" name="name" placeholder="Search Product" value={selectedName} onChange={handelnameChange} className="p-2 border border-gray-300 rounded-md dark:border-strokedark dark:bg-boxdark dark:text-white"/>
